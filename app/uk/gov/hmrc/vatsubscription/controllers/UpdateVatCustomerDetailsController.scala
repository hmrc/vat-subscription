...
class UpdateVatCustomerDetailsController @Inject()(VatAuthorised: VatAuthorised,
  ...
  def updateVatReturnPeriod(vrn: String, newPeriod: String): Action[AnyContent] = VatAuthorised.async(vrn) {
    implicit user =>
      ReturnPeriod(newPeriod) match {
        case Some(period) =>
        ...
        case _ =>
          Future.successful(BadRequest(Json.toJson(ErrorModel("INVALID_RETURN_PERIOD", s"The supplied return period '$newPeriod' is invalid."))))
      }
  }
}
