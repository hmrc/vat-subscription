...
class Features @Inject()(implicit config: Configuration) extends BaseFeature {
  ...
  lazy val enableAnnualAccounting = new Feature(s"$featureSwitch.enableAnnualAccounting")
  lazy val newStatusIndicators = new Feature(s"$featureSwitch.newStatusIndicators")
